...
## PLEASE insert the model that you want to use
## The wda value determines which model you want to run, e.g., ("WDCNN","WDTCNN","WDMTCNN")
wda = 

## Scale the data
scaler = StandardScaler()
x_train2 = scaler.fit_transform(x_train.reshape(-1, x_train.shape[-1])).reshape(x_train.shape)
x_test2 = scaler.transform(data.x_test.reshape(-1, data.x_test.shape[-1])).reshape(data.x_test.shape)

y_train2 = data.y_train
y_test2 = data.y_test
num_classes = data.nclasses

## Determine pooling type based on model architecture:
def pooling_type(wda):
    if wda == "WDCNN":
        x = MaxPooling1D(strides=2)
    else:
...
def wd_layers(nr_blocks,first_layer,seq_len,sens,stride,kernel,wda,first_kernel_size,first_stride):
    ...
    for i in range(nr_blocks):
        ...
        xx = Conv1D(filters=32,kernel_size=kernel,strides=stride,padding='same')(xx)
        xx = BatchNormalization()(xx)
        xx = Activation('relu')(xx)
        xx = pooling_type(wda)(xx)
        xx = Dropout(float(sys.argv[6]))(xx)
        ...
        ls = ['same','same','valid']
        for j in ls:
            xx = Conv1D(filters=64,kernel_size=kernel,strides=stride,padding=j)(xx)
            xx = BatchNormalization()(xx)
            xx = Activation('relu')(xx)
            xx = pooling_type(wda)(xx)
            xx = Dropout(float(sys.argv[6]))(xx)
...
def full_model_WDMTCNN(x_train2,x_test2,y_train2,y_test2):
    ...
    earlystop = EarlyStopping(monitor= 'val_loss', min_delta=0 , patience=int(sys.argv[7]), verbose=0, mode='auto',restore_best_weights=True)
    
    # Initialize loop for every kth fold
    for train, test in kf.split(x_train2, y_train2['label']): # Must specify y StratifiedKFold for 
        model_checkpoint = ModelCheckpoint('WD_cwru_model.h5', monitor='val_loss',verbose=1, save_best_only=True)
        fold+=1
        print(f"Fold #{fold}")
        ...
        ## Load best model
        wdcnn_model = load_model('./WD_cwru_model.h5',custom_objects={'attention': attention})
    ...
    return(oos_test_prob, oos_test_y, aggregated_score, aggregated_test_score, earlystop.patience, 
